...
\noindent On the econometric side, static Ordinary Least Squares (OLS) models, while useful for establishing baseline U-curve relationships, typically suffer from severe residual serial correlation in high-frequency daily data. To address this, dynamic distributed-lag models capture the fact that weather shocks propagate over several days as households and firms adjust heating and cooling behaviour. The ARDL framework \citep{pesaran1997} is highly attractive because it accommodates variables integrated of different orders and resolves the functional form misspecifications common in static load models.   To avoid overfitting and ensure robust inference, modern time-series econometrics relies on the general-to-specific (GTS) modelling approach. By starting with a comprehensively parameterised model and sequentially eliminating statistically insignificant terms via nested $F$-tests, GTS ensures that the final parsimonious model retains only the structural parameters that truly drive electricity demand. For operational forecasting, \citet{weron2014} emphasises the value of exogenous weather and calendar variables-a conclusion we further validate in our hourly ARIMA exercise.
...
\begin{table}[H]
  ...
  \scriptsize
  \input{tables/tab_stationarity.tex}
  \\[0.5em]
...
\begin{table}[H]
  ...
  \small
  \input{tables/tab_baseline_diag.tex}
\end{table}
...
\begin{table}[H]
  ...
  \small
  \input{tables/tab_gts_ardl_de.tex}
\end{table}
...
\begin{table}[H]
  ...
  \small
  \input{tables/tab_baseline_ols.tex} 
\end{table}
...
The diagnostic results in Table~\ref{tab:ardl_diag} confirm the structural superiority of the seasonal ARDL framework over static OLS. Notably, the Ramsey RESET test fails to reject the null for Germany (0.572), Spain (0.625), and France (0.968), demonstrating that the inclusion of dynamic lags successfully resolves the functional form misspecification that plagued the baseline models.   Furthermore, Table~\ref{tab:ardl} shows that day-to-day serial correlation is substantially mitigated, with BG(1) $p$-values improving significantly (e.g., DE passing at 0.031). Figure~\ref{fig:ardl_acf} visually confirms this reduced lag-1 autocorrelation relative to OLS. However, persistent BG(7) rejection reflects complex weekly load seasonality, such as holidays, shift-work cycles, and school calendars, that a simple lag structure cannot fully absorb. Consequently, while the ARDL correctly models the temperature dynamics, we appropriately rely on HAC-robust standard errors for safe inferential testing.\footnote{Inference utilizes Newey-West HAC robust standard errors via the \texttt{sandwich::vcovHAC} package.}
...
\begin{table}[H]
  ...
  \label{tab:ardl_diag}
  \small
  \input{tables/tab_ardl_diag.tex}
\end{table}
...
\noindent Because the dependent variable is specified in log differences ($\Delta \ln(L_t)$) and the weather regressor in temperature changes ($\Delta T_t$), the short-run temperature effect ($\sum \hat{\beta}_j$) acts as a cumulative short-run elasticity. Specifically, this value indicates the percentage change in daily electricity demand resulting from a sustained $1^\circ$C increase in temperature over the distributed lag window. For example, Germany's estimate of $-0.0073$ implies that a $1^\circ$C warming shock reduces daily demand by approximately 0.73\%, while France's higher sensitivity ($-0.0197$) yields a nearly 2\% reduction, reflecting its heavily electrified residential heating sector.
...
\begin{table}[H]
  ...
  \input{tables/tab_ardl_arima.tex}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{1.2cm}p{3.2cm}p{2.2cm}p{6.5cm}}
    ...
    H2 & ARDL reduces autocorrelation vs.\ OLS & Partially supported & BG(7) rejects under OLS for all focus countries. The seasonal ARDL successfully neutralises day-to-day momentum (BG(1) passes or improves significantly), but BG(7) remains $\approx 0$ due to complex weekly institutional schedules. Thus, autocorrelation is reduced but not completely eliminated, necessitating HAC-robust standard errors. \\
        
    H3 & Cross-country heterogeneity in short-run temp effects & Partially supported &
    Magnitudes differ substantially (FR: $-0.020$; FI: $-0.009$; DE: $-0.007$). All short-run sums are negative, including ES ($-0.009$). While unexpected for a cooling-dominated country, Spain's sample mean temperature is a mild 15.5$^\circ$C, meaning most days still operate on the heating arm of the U-curve. Daily averaging masks intra-day cooling peaks, resulting in a net negative daily elasticity. \\
...
\begin{enumerate}
  ...
  \item Download the Zenodo archive and unpack into a repository.
  \item Install dependencies: \texttt{pip install -r requirements.txt} and \texttt{Rscript setup-r.R} (or \texttt{renv::restore()}).
  \item From \texttt{root-repository/}: run \texttt{make all}. This regenerates all CSV tables, figures, and the compiled \texttt{report.pdf} from the archived merged panel.
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Population.} \texttt{codes/01\_download\_population.py} fetches Eurostat \texttt{demo\_pjan} totals and builds GPW v4 grid weights (\texttt{gpw\_grid\_weights.parquet}). Place \path{gpw_v4_population_2020.tif} from NASA SEDAC in \texttt{data/} for full spatial weighting.
  ...
  \item \textbf{Merge.} \texttt{codes/04\_merge\_data.py} harmonises ERA5 streams, applies population-weighted weather aggregation, and joins to hourly load $\rightarrow$ \path{final_integrated_u_curve_dataset.parquet}.
